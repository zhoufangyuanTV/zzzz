...
int main()
{
	...
	for(int i=2;i<=n;i++)
	{
		...
		if(i==n)
		{
			cout<<Decimal(st).to_string(p+1)<<endl;
			return 0;
		}
	}
	if(m>n)m=n;
	if(p<=40)
	{
		...
		for(int i=1;i<=n;i++)
		{
			for(int j=1;j<=i&&j<=m;j++)
			{
				...
				{
					s+=a[k];
					Decimal ss=(s+f[k-1][j-1])/(i-k+2);
					if(f[i][j]<ss)f[i][j]=ss;
				}
			}
		}
		cout<<f[n][m].to_string(p+1)<<endl;
	}
	else
	{
		for(int i=0;i<=n;i++)g[i][0]=st;
		for(int i=1;i<=n;i++)
		{
			for(int j=1;j<=i&&j<=m;j++)
			{
				...
				{
					s+=a[k];
				    double ss=(s+g[k-1][j-1])/(i-k+2);
					if(g[i][j]<ss)g[i][j]=ss;
				}
			}
		}
		cout<<g[n][m]<<endl;
